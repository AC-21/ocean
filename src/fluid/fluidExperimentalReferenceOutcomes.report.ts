...
const timeoutMs = Number(process.env.OCEAN_LAB_EXPERIMENTAL_REFERENCE_TIMEOUT_MS || 90_000);
const root = process.cwd();
const outPath = process.env.OCEAN_LAB_EXPERIMENTAL_REFERENCE_OUT || "reports/fluid-experimental-reference-outcomes-latest.json";
...
const launchMode = process.env.OCEAN_LAB_EXPERIMENTAL_REFERENCE_TARGET === "source" ? "electron-source" : "packaged-app";
const userDataRoot = await mkdtemp(path.join(tmpdir(), "ocean-lab-experimental-reference-"));
...
try {
  ...
  const report = createFluidExperimentalReferenceOutcomesReport({
    ...
    launchMode,
    noFullGridReadbackPerFrame: cases.every((entry) => entry.telemetry.noFullGridReadbackPerFrame && entry.telemetry.particlesNoFullGridReadbackPerFrame === true) && finalTelemetry.noFullGridReadbackPerFrame && finalTelemetry.particlesNoFullGridReadbackPerFrame === true,
    pageErrors,
    preferredTier,
    runtimeGrid,
    runtimeGridOverride,
  ...
  console.log(`Fluid experimental reference outcomes report written to ${outPath}`);
  console.log(`- runtime: ${launchMode}`);
  console.log(`- capability grid: ${report.summary.capabilityGrid}`);
  console.log(`- live grid: ${report.summary.liveGrid}`);
  ...
  assert.equal(launchMode, "packaged-app", "FG-40 evidence must use the packaged app by default");
  assert.equal(report.gate, "G-FG-40", "FG-40 evidence must use the experimental reference outcomes gate id");
  assert.deepEqual(report.failures, [], `FG-40 failures:\n${report.failures.join("\n")}`);
} finally {
...
function gridFromTelemetry(telemetry: FluidReferenceCanvasTelemetry): FluidWaterRuntimeGridDimensions {
  const [cellsX, cellsY] = (telemetry.grid ?? "0x0").split("x").map((entry) => Number(entry));
