...
import {
  ...
  type FluidInstalledHighResolutionDesktopVisibilityReport,
  type InstalledHighResolutionDesktopVisibilityPixelProbe,
} from "./fluidInstalledHighResolutionDesktopVisibility";

type HighResolutionFloatSinkEvidence = {
  gate: string;
  pass: boolean;
  runtime: {
...
type WindowState = FluidInstalledHighResolutionDesktopVisibilityReport["window"];

const execFileAsync = promisify(execFile);
const appName = "Ocean Impact Lab";
const timeoutMs = Number(process.env.OCEAN_LAB_INSTALLED_HIGH_RESOLUTION_DESKTOP_VISIBILITY_TIMEOUT_MS || 30_000);
const stabilizationMs = Number(process.env.OCEAN_LAB_INSTALLED_HIGH_RESOLUTION_DESKTOP_VISIBILITY_STABILIZE_MS || 2_500);
const outPath =
  process.env.OCEAN_LAB_INSTALLED_HIGH_RESOLUTION_DESKTOP_VISIBILITY_OUT ||
  "reports/fluid-installed-high-resolution-desktop-visibility-latest.json";
const screenshotPath =
  process.env.OCEAN_LAB_INSTALLED_HIGH_RESOLUTION_DESKTOP_VISIBILITY_SCREENSHOT ||
  "reports/fluid-installed-high-resolution-desktop-visibility-latest.png";
const highResolutionEvidencePath =
  process.env.OCEAN_LAB_INSTALLED_HIGH_RESOLUTION_DESKTOP_VISIBILITY_IN ||
...
const highResolutionEvidence = await readJson<HighResolutionFloatSinkEvidence>(highResolutionEvidencePath);
...
const report = createFluidInstalledHighResolutionDesktopVisibilityReport({
  highResolutionEvidence: {
    gate: highResolutionEvidence.gate,
    liveGrid: highResolutionEvidence.runtime.liveGrid,
    pass: highResolutionEvidence.pass,
    renderer: highResolutionEvidence.runtime.renderer,
    sourcePath: highResolutionEvidencePath,
    visual: highResolutionEvidence.visual?.pixelProbe ?? null,
    waterContext: highResolutionEvidence.runtime.waterContext,
  },
  launcher: {
    path: launcherPath,
  ...
  process: processInfo,
  storage: {
    defaultStorage: highResolutionEvidence.storage.defaultStorage,
    profileHadRuntimeGrid: highResolutionEvidence.storage.profileHadRuntimeGrid,
    readByMainProcess: highResolutionEvidence.storage.readByMainProcess,
  },
  viewportProbe: {
    crop,
    pixelProbe,
...
console.log(`Fluid installed high-resolution Desktop visibility report written to ${outPath}`);
console.log(`- source: ${report.highResolutionEvidence.gate}, ${report.highResolutionEvidence.liveGrid}`);
...
assert.equal(report.gate, "G-FG-44", "FG-44 evidence must use the installed high-resolution Desktop visibility gate id");
assert.deepEqual(report.failures, [], `FG-44 failures:\n${report.failures.join("\n")}`);
...
async function installedProcessInfo(): Promise<FluidInstalledHighResolutionDesktopVisibilityReport["process"]> {
...
): InstalledHighResolutionDesktopVisibilityPixelProbe {
  let samples = 0;
  let opaqueSamples = 0;
  let lumaTotal = 0;
